...
proc_description_CBCwDIFF <- subset(hep_c_labs, proc_description =="CBC W/ DIFF & PLATELETS") #148,461 rows
proc_description_CBC <- rbind(proc_description_CBC,proc_description_CBCwDIFF)

length(unique(proc_description_CBC$PATIENT_ID))  #1959 unique patients
...
cbc <- cbc[!duplicated(cbc),] #33,152

##Freq of CBC labs for each patient
cbc_freq <- as.data.frame(table(cbc$PATIENT_ID))  #1959 unique patients
...
less_than_28_days_freq <- as.data.frame(table(less_than_28_days$PATIENT_ID)) #1345 patients
...
consec_greater_than_3 <- as.data.frame(table(consec_greater_than_3$PATIENT_ID))#985 patients
...
proc_description_HIV <- hep_c_labs[grep("HIV", hep_c_labs$proc_description), ] #3339
proc_description_HIV <- proc_description_HIV[(order(proc_description_HIV$PATIENT_ID, proc_description_HIV$order_dttm_ds)),]

length(unique(proc_description_HIV$PATIENT_ID))  #719 unique patients
...
treatment_patients <- subset(hep_c_patients, !is.na(Num_of_CBC_Labs) &
                               ...
                               !is.na(Num_of_Hep_B_Labs))  #497 unique patients
...
#write.csv(meds_3consec, "Z:/meds_3consec.csv")

#read in meds_3consec
meds_3consec<-read_delim("Z:/meds_3consec.csv", ",", escape_double = FALSE, trim_ws = TRUE)
...
#write.csv(meds_3consec_uniques, "Z:/meds_3consec_uniques.csv")

#also look at meds for all 2485 hep C patients
meds_2485 <- subset(reduced_meds, reduced_meds$PATIENT_ID %in% hep_c_patients$PATIENT_ID)

meds_2485_uniques <- unique(meds_2485[c("Thera_class_c","Thera_class","Pharm_class_c","Pharm_class","Pharm_subclass_c","Pharm_subclass")])

#write.csv(meds_2485_uniques, "Z:/meds_2485_uniques.csv")
